import cleaningsInProgressData from '@/cleaningsInProgressData'
...
const actions = {
    async progressCleaning({commit}) {
        ...
        commit('replace_loading_cleanings_in_progress', { loading: true })
        setTimeout(() => {
            const data = cleaningsInProgressData
            commit('set_progress', data)
            commit('replace_loading_cleanings_in_progress', { loading: false })
        }, 500);
    },
...
const mutations = {
    replace_loading_cleanings_in_progress(state, { loading }) {
